...
for _, c in clients.sort_values("total_aum_usd", ascending=False).iterrows():
    print(
        f"{c.client_id:<9}{c.client_name[:26]:<28}{c.total_aum_usd / 1e6:>9.1f}  "
...
piv = (
    market[market.series_id.isin(watch)]
...
for pid in portfolios.loc[portfolios.client_id == cid, "portfolio_id"]:
    ...
    print(
        f"  {pid}  {row.portfolio_name}  "
        f"({row.service_model}, {row.mandate_name}, {row.base_currency})"
    )
...
top = h.groupby("instrument_name").market_value_usd.sum().sort_values(ascending=False).head(5)
